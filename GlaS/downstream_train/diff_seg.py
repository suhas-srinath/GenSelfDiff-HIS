...
from losses import CELoss, Tversky_Loss, FLoss, SSLoss
...
class ElasticDeformation(object):
    ...
    def __call__(self, image):
        ...
        if random.random() > self.p:
            # Apply your custom augmentation logic here
            image = self.deform_augmentation(image)

        return image

    def deform_augmentation(self, image):
        ...
        np.random.seed(2023)
        sigma = 0.05*S
        dx = gaussian_filter(np.random.uniform(-1, 1, (shape[0], shape[1])), sigma, mode="constant", cval=0) * alpha
        dy = gaussian_filter(np.random.uniform(-1, 1, (shape[0], shape[1])), sigma, mode="constant", cval=0) * alpha

        # Create meshgrid
        x, y = np.meshgrid(np.arange(shape[1]), np.arange(shape[0]))
        indices = np.reshape(y+dy, (-1, 1)), np.reshape(x+dx, (-1, 1))

        # Map coordinates from input image to output image
        deformed_image = np.zeros_like(image)
        for channel in range(image.shape[-1]):
            channel_image = image[..., channel]
            deformed_image[..., channel] = map_coordinates(channel_image, indices, order=1, mode='reflect').reshape(shape)
...
def main(gpu, args):
    ...
    train_dataset = GlaS_Dataset(TRAIN_IMAGE_DIR,
                                  img1_list[train_index], train_labels[train_index],
                                  transform=transformations)
    eval_dataset = GlaS_Dataset(TRAIN_IMAGE_DIR,
    ...
    model = SegNet(dim=64, channels=3, num_classes=NUM_CLASSES).to(gpu)
    ...
    if args['load_from_chkpt'] is not None:
        ...
        # new_pretrained_dict = {k: v for k, v in pretrained_dict.items() if k[:14] == 'net.init_conv.' or
        #                        k[:13] == 'net.time_mlp.' or k[:10] == 'net.downs.'}
    ...
    axes.set_title(
        f'Training and Validation Loss (pretrained model = diffusion, loss = (SS + Focal) Loss, '
...
if __name__ == '__main__':
    ...
    args['load_from_chkpt'] = ''  # mention the path for checkpoint
