...
BATCH_SIZE = 1
NUM_CLASSES = 4
TEST_IMAGE_DIR = '../downstream_train/test/images'
...
def eval_epoch(test_loader, model, num_classes, device):
    ...
    with torch.no_grad():
        model.eval()
        val_dice = np.array([0.0, 0.0, 0.0])
        precisions = np.array([0.0, 0.0, 0.0])
        senstivities = np.array([0.0, 0.0, 0.0])
        specificities = np.array([0.0, 0.0, 0.0])
        accuracies = np.array([0.0, 0.0, 0.0])
        fscores = np.array([0.0, 0.0, 0.0])
        ...
        for img, target_label in p_bar:
            img = img.to(device)
            target_label1 = target_label.long().to(device)
...
def main():
    ...
    label_file = loadmat('../downstream_train/test/label_patches.mat')
    train_labels = label_file['data']

    test_dataset = HN_Dataset(TEST_IMAGE_DIR, img1_list, train_labels,
                                 transform=transformations)

    test_loader = DataLoader(test_dataset, batch_size=BATCH_SIZE,
                             shuffle=False, drop_last=False, num_workers=4)

    path_train = './downstream_train/snapshots/diff_quadratic_general_SSFL_randomseed/8_0.0001_36_0.57715.pth'
    snapshot = torch.load(path_train)
    print(path_train)

    model = SegNet(dim=64, channels=3, num_classes=4).to(DEVICE)
